...
# generate files for Idealized Maud Rise problem.
# Gustavo Marques
...
def parseCommandLine():
  ...
  parser.add_argument('-nz', type=int, default=60,
      help='''Number of model layers (default = 60).''')

  parser.add_argument('-W', type=float, default=1000.,
      help='''Domain width in the x direction (km). Default is 1000.''')

  parser.add_argument('-cshelf_lenght', type=float, default=200.,
      help='''Continental shelf lenght in the y direction (km). Default is 200.''')

  parser.add_argument('-slope_lenght', type=float, default=80.,
      help='''Continental shelf slope lenght in the y direction (km). Default is 80.''')
  ...
  parser.add_argument('-L', type=float, default=1000.,
      help='''Domain lenght in the y direction (km). Default is 1000.''')

  parser.add_argument('-max_depth', type=float, default=5.0e3,
      help='''Maximum ocean depth (m). Default is 5E3.''')
...
def make_topo(x,y,args):
   ...
   L = args.L  # domain lenght in y dir, in km
   delta = 0.6 # non-dim height
   Lx = 100.0
   Ly = 100.0
   [X,Y] = np.meshgrid(x,y)
   nx = len(x); ny = len(y)
   ...
   for j in range(ny):
      for i in range(nx):
          D1[j,i] = Hs + 0.5 * (H-Hs) * (1.0 + np.tanh((Y[j,i]*1.0e3 - Ys)/Ws))
   ...
   for j in range(ny):
      for i in range(nx):
        x1 = x[i]/W - 0.5
        y1 = y[j]/L - 0.65
        D2[j,i] = H * (1.0 - delta * np.exp(-(rLx*x1)**2 -(rLy*y1)**2))
        D[j,i] = np.min([D1[j,i], D2[j,i]])
